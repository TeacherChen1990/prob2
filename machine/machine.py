...
class DataPath:
    ...
    def set_value_do(self, index: int):
        self.do_position = index

    def set_value_loop(self, index: int):
        self.loop_position = index

    def get_value_loop(self) -> int:
        return self.loop_position

    def get_value_do(self) -> int:
        return self.do_position
    ...
    def action(self, f):
        ...
        if f == ALU.add:
            ...
            if self.left > 0 and self.right < 0 and f(self.left, self.right) < 0:
                ...
                if f(self.left, self.right) < 0:
                    self.nzvc = N
        r = f(self.get_left(), self.get_right())
        return r
...
class CPU:
        ...
        if ins.instruction == InstructionType['HLT']:
            ...
            if ins.instruction == out:
                if arg == "OUTPUT":
                    ...
                    self.datapath.output_buffer.write_pointer += 1
                    self.tick_tick()
            elif ins.instruction == InstructionType['LD']:
                if arg == 'INPUT':
                    # index -> AR， io->ac
            ...
            # 循环起点
            if ins.instruction in {InstructionType.DO, InstructionType.LOOP, InstructionType.LEAVE}:
                ## Decoder -> IP
                arg = ins.instruction.value
                if ins.instruction == InstructionType.LEAVE:
            ...
            # 循环结尾(不代表离开循环)
            elif ins.instruction == InstructionType.LOOP:
                ...
                self.tick_tick()
            # 离开循环 跳转到LOOP的下一行代码
            elif ins.instruction == InstructionType.LEAVE:
                # AC->BR make sure that hello of function is saved
                ...
                self.tick_tick()
            elif ins.instruction == InstructionType.IF:
                self.alu.put_right(self.addressing(ins))
                # AC - arg to check nzcv -> PS
                self.alu.put_left(self.datapath.get_value_register("AC"))
                self.alu.action(ALU.min)
    ...
    def run(self):
        ...
        result = ""
        while True:
            r, ins = self.run_ins(position=self.position[-1])
            if ins.instruction == InstructionType["OUTPUT"]:
                if ins.args[0] != "OUTPUT":
                    result = result + re.sub(r"\"", "", ins.args[0])
            if r == 1:
                break
        print("Output:")
        first = True
